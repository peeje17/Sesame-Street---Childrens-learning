...
I expect socioeconomic status to have a positive effect on both correlations (both encourgement and regular viewers) because Sesame Street originally targeted children
who lived in cities and had a low socioeconomic background (Lovelace, 1990). Based on this, I have I developed the following hypothesis:
...
I will now explore wheter the other variables in the dataset alters the treatment effect as shown in figure 2 using a OLS regression. 
...
2= 4-year-olds better off children from the suburbs 
...
For the regular varible I find no meaningfull results in relation sites interaction on regular viewers knowledage about numbers (not shown). The regular varibale is still significant. More interesting is the interaction between the encorugement variable and the site variable which now shows a siginificant result.
...
The overall conclusion and interesting result based on this dataset, is that regular viewers of the TV show Sesame Street have a better knowledge of numbers than non-regular viewers (H2). The internal validity is high because of the many control variables which included in the study. However, the external validity more modest because the dataset only contains 240 observations. The first hypothis concerning the encourgement from parents to let their children watch the TV program Sesame Street showed no significant effect. The last hypothsis analyzed whether low socioeconomic status positively moderates the effect of the TV program Seseame Str. on children's knowledge about numbers with the use clusted standard errors. Here the results are more muddy. There was no significant result of diffenrent groups of regular viewers, but with encourgement and espicially with group 1 to 2 showed a postive significant result. The postive relation of beeing encourge to watch Sesame Str. and going from group 1 (disadvangted children from urban centers) and to group 2 (better of children from the suburbs), contadicts the expectation from H3, which anticipated the program would be more effectful for disadvangted children. This results must also be seen in the light of the very poor site variable, where serval dimensions are included and therefore has a change for muddy results. For example are there differences between the groups concerning age, race and geography, which then makes it difficult to distingush the variables from each other. 
