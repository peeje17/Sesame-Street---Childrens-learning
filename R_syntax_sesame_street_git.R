...
summary(bal.test2) # gender not significant 


bal.test3 <- aov(setting_new~view_enc, data=df)
summary(bal.test3) # setting is significant
describeBy(df, df$setting_new)


bal.test4 <- aov(site~view_enc, data=df)
summary(bal.test4) # site is not significant 
describeBy(df, df$site)


bal.test6 <- aov(regular~view_enc, data=df)
summary(bal.test6) # regular is significant  
describeBy(df, df$regular)



# Making gender, setting, view_enc, site and regular to factor/categorical variables
...
df$regular <- as.factor(df$regular)
                                                         ...
                                                         ### --- Average treatment effect for view_enc --- ###
...
ggplot(plot.data.regular, aes(regular, mean)) +        
  ...
  labs(x= "Regular viewer of Sesame Street 0 = No 1 = Yes", y = "Knowledge about numbers")
                                                ...
                                                ### --- Regular effect on post_numbers - using OLS --- ### 
